...
import functools
from typing import Callable, Generic, Iterator, List, Optional, Type, TypeVar, Union
  ...
  * All created protos are final. You can use them as a submessage inside
  ...
  Returns:
    ...
    * A single positional argument with a plain message of the given
      ...
      defaults to the default/empty message for that type.
    * A series of keyword arguments, which correspond to fields of the proto
      ...
      map fields must be passed as dicts. However, the values of these fields
      may be passed as placeholders. In particular, other CreateProtoPlaceholder
  ...
  def _validate_and_transform_field(
    ...
    if descriptor.label == descriptor_lib.FieldDescriptor.LABEL_REPEATED:
      ...
      if value is None or isinstance(value, placeholder_base.Placeholder):
        return value  # pytype: disable=bad-return-type (false alarm)

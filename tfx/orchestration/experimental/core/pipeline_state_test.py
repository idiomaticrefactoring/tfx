...
class PipelineIRCodecTest(test_utils.TfxTest):
  ...
  def test_encode_decode_file_not_exist(self):
    pipeline_encoded = {'pipeline_ir_url': '/fake/file_path'}
    with self.assertRaises(fileio.NotFoundError):
      pstate._PipelineIRCodec().decode(json_utils.dumps(pipeline_encoded))
